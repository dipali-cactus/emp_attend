        ...
        <div class="col-xl-12 col-lg-7">
          <div class="card shadow mb-4" style="min-height: 543px">
            ...
            <div class="card-body">
              ...
              <?php else : ?>
                <?php if ($in === false) : ?>
                  <?= form_open_multipart(base_url('attendance'), ['method' => 'post']) ?>
                  <div class="row mb-3">
                    <div class="col-lg-5">
                      <input type="hidden" name="work_shift" value="<?= esc($account['shift']); ?>">
                      <label for="work_shift" class="col-form-label">Work Shift</label>
                      <input class="form-control" type="text" placeholder="<?= date("h:i A", strtotime($startTime)); ?> - <?= date("h:i A", strtotime($endTime)); ?>" value="<?= date("h:i A", strtotime($startTime)); ?> - <?= date("h:i A", strtotime($endTime)); ?>" readonly>
                    </div>
                ...
                <?php else : ?>
                  ...
                  <?php if ($disable === false) : ?>
                    <div class="row">
                      <a href="<?= base_url('attendance/checkout') ?>" onclick="return confirm('Time Out now? Make sure you are done with your work!')" class="btn btn-danger bg-gradient-danger rounded-pill px-5 btn-lg mx-auto">
